...
class RGCN(torch_geometric.nn.MessagePassing, ABC):
    ...
    def compute_relation_weights(self, relation_ids: Tensor):
        relation_coefficients = torch.index_select(
            input=self.coefficients, dim=0, index=relation_ids
        )  # relation coefficients corresponding to relation_ids, (num_given_relations, num_bases)
        relation_weights = torch.matmul(
            relation_coefficients, self.bases.view(self.num_bases, -1)
        ).view(relation_ids.size()[0], self.in_dimension, self.out_dimension)
    ...
    def aggregate(self, inputs: Tensor, index: Tensor, edge_type: Tensor = None, ptr: Optional[Tensor] = None, dim_size: Optional[int] = None) -> Tensor:
        ...
        )  # relation count for all entities, (num_entities, num_relations); note: num_entities = max(entity ids in index) + 1;
        relation_count_for_edges = torch.index_select(
            input=relation_count_for_entities, index=index, dim=0
        )  # relation count for target entities, (num_target_entities, num_relations); note: num_target_entities == num_edges;
